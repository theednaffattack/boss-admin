...
if (process.env.VIRTUAL_PORT) {
    port = parseInt(process.env.VIRTUAL_PORT);
} else {
    port = 9000;
    console.log("UH OH, DEFAULT PORT (9000) USED", port);
}
...
export async function server(): Promise<void> {
    ...
    if (process.env.VIRTUAL_PORT) {
        httpServer.listen(parseInt(process.env.VIRTUAL_PORT, 10), () => {
            if (httpServer) {
